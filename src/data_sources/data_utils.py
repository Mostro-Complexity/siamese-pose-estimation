...
SH_NAMES = [''] * 16
# sh -> 2dbaseline: 2d = sh[[6, 2, 1, 0, 3, 4, 5, 7, 8, 9, 13, 14, 15, 12, 11, 10]]
SH_NAMES[0] = 'RFoot'
SH_NAMES[1] = 'RKnee'
SH_NAMES[2] = 'RHip'
SH_NAMES[3] = 'LHip'
...
def load_3d_data(bpath, subjects, actions, frame_density=None):
    ...
    for subj in subjects:
        for action in actions:
            dpath = os.path.join(bpath, 'S{0}'.format(
                subj), 'MyPoses/3D_positions', '{0}*.h5'.format(action))
...
def find_pose_files(data_dir, subtype, subjects, actions):
    ...
    for subj in subjects:
        for action in actions:
            dpath = os.path.join(data_dir, 'S{0}'.format(
                subj), '{0}/{1}*.*'.format(subtype, action))
            ...
            if subj == 11 and action == 'Directions':  # <-- this video is damaged
                assert files_found in [7, 43], "Expecting 7 or 43sequences, found {0} instead. S:{1} {2}".format(
                    files_found, subj, action)
            else:
                assert files_found in [8, 44], "Expecting 8 or 44 sequences, found {0} instead. S:{1} {2}".format(
                    files_found, subj, action)
...
def load_stacked_hourglass(data_dir, subtype, subjects, actions, frame_density=None):
    ...
    SH_TO_GT_PERM = np.array([SH_NAMES.index(h)
                              for h in H36M_NAMES if h != '' and h in SH_NAMES])
...
def load_augmented_pose(data_dir, subjects, actions, frame_density=None):
    ...
    files = find_pose_files(
        data_dir, 'StackedHourglassFineTuned240Augmented', subjects, actions)
...
def project_to_cameras(poses_set, cams, ncams=4, frame_density=None):
    ...
    for t3dk in sorted(poses_set.keys()):
        ...
        for cam in range(ncams):
            ...
            # This filtering must be done here, otherwise a memory leak occurs
            if frame_density is not None:
                # copy needed to avoid views and thus holding on to memory
                pts2d = pts2d[::frame_density, :].copy()
